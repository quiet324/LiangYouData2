...
rule.hour = [0, 1, 6, 7, 9, 11, 12, 13, 18, 21];
rule.minute = 42;
...
var j = schedule.scheduleJob(rule, function() { // rule hour at 5 minutes
    ...
    if (!fs.existsSync('../../vof/old' + vofFileName)) {
    ...
    cos.sliceUploadFile({
        Bucket: 'dailyaudio', // 替换为你的Bucket名称
        Region: 'ap-chengdu', // 设置COS所在的区域，对应关系: 华南->cn-south, 华东->cn-east, 华北->cn-north
        Key: audioFilesForCOSFileNameDone.substring(audioFilesForCOSFileNameDone.lastIndexOf('/')), // 设置上传到cos后的文件的名称
        FilePath: audioFilesForCOSFileNameDone // 设置要上传的本地文件
    }, function(err, data) {
        sync = false;
    ...
    cos.sliceUploadFile({
        Bucket: 'dailyaudio', // 替换为你的Bucket名称
        Region: 'ap-chengdu', // 设置COS所在的区域，对应关系: 华南->cn-south, 华东->cn-east, 华北->cn-north
        Key: audioFilesForCOSFileName.substring(audioFilesForCOSFileName.lastIndexOf('/')), // 设置上传到cos后的文件的名称
